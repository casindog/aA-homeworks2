...
def find_angelina
  ...
  # Actor.select(*).where("name = ?", "Angelina Jolie") 
...
def top_titles
  ...
  # Movie.select(:title).where(["name >= ?", 9])
  Movie
    .select(:id, :title)
    .where('score >= 9')
...
def star_wars
  ...
  # Movie.select(:id, :title, :year).where(["name like", "Star Wars%"])
  Movie
...
def below_average_years
  ...
  # Movie.select(:year, count(:title) as bad_movies).where([ratings < 5].group(:year).order(Desc)
  Movie
...
def alphabetized_actors
  ...
  Actor.select(name).order(ASC).limit(10)
  Actor 
    .select(:name)
    .order('name ASC')
    .limit(10)
...
def pulp_fiction_actors
  ...
  #ohhh, yeah. it's joining on the association! not table. fuucccckkkk
  Actor.select(actors.id, actors.name).joins(:actors).where(movies.title = 'Pulp Fiction')

  Actor
    .select(actors.id, actors.name)
    .joins(:movies) #this should be an association
...
def uma_movies
  ...
  Movie
    ...
    .joins(:actors) #this should be an association. 
